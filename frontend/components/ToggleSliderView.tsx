    ...
    <div className="w-full">
      ...
      >
        <motion.div
          ...
          layout
          initial={{ x: 0 }}
          animate={{ x: view === "map" ? 0 : "100%" }}
          transition={{ type: "spring", stiffness: 200, damping: 20 }}
        />
        <div className={`w-1/2 text-center z-10 text-sm font-bold ${view === "map" ? "text-red-700" : "text-gray-700"}`}>Map</div>
        <div className={`w-1/2 text-center z-10 text-sm font-bold ${view === "cards" ? "text-red-700" : "text-gray-700"}`}>Card</div>
      </div>
      <div className="mt-4 w-full">
        {view === "map" ? <MapView /> : <ViolenceCardGrid />}
      </div>
    </div>
  );
}
