...
export default function ViolenceQueryCardGrid({ api_query, violenceData, setViolenceData }: ViolenceQueryCardGridProps) {
    ...
    useEffect(() => {
        const fetchViolence = async () => {
            try {
                setLoading(true);
                const response = await fetch(`http://127.0.0.1:5002/api/incidents?${api_query}`);
                    ...
                    <>
                        {violenceData && 
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {violenceData.incidents.map((incident, index) => <IncidentCard key = {index} incident = {incident}/>)}
                            </div>
                        }
